...
class MID():
    ...
    def train(self):
        for epoch in range(1, self.config.epochs + 1):
            ...
            if epoch % self.config.eval_every == 0 and epoch >= 40:
                self.model.eval()
                ...
                for i, scene in enumerate(self.eval_scenes):
                    ...
                    for t in tqdm(range(0, scene.timesteps, 10)):
                        ...
                        batch = get_timesteps_data(env=self.eval_env, scene=scene, t=timesteps, node_type=node_type, state=self.hyperparams['state'],
                                       ...
                                       min_ht=self.config.h_t, max_ht=self.config.h_t, min_ft=self.config.f_t,
                                       max_ft=self.config.f_t, hyperparams=self.hyperparams)
                        if batch is None:
                            continue
                        test_batch = batch[0]
                        nodes = batch[1]
                        timesteps_o = batch[2]
                        traj_pred = self.model.generate(test_batch, node_type, num_points=self.config.f_t, sample=20,bestof=True) # B * 20 * 12 * 2
                        ...
                        batch_error_dict = evaluation.compute_batch_statistics(batch_number,
                                                                               predictions_dict,
                                                                               scene.dt,
                                                                               max_hl=max_hl,
                                                                               ph=ph,
                ...
                if ade < self.best_ade:
                    ...
                    torch.save(best_checkpoint, osp.join(self.model_dir, f"best_model_epoch{epoch}.pt"))
                    self.log.info(f"New best model saved with ADE: {ade}")
    ...
    def eval(self, sampling, step):
        ...
        for i, scene in enumerate(self.eval_scenes):
            ...
            for t in tqdm(range(0, scene.timesteps, 10)):
                timesteps = np.arange(t,t+10)
                # 每次从一个范围内取起止点的连续行人history和future等，由于minhis限制，最低为7
                batch = get_timesteps_data(env=self.eval_env, scene=scene, t=timesteps, node_type=node_type, state=self.hyperparams['state'],
                               pred_state=self.hyperparams['pred_state'], edge_types=self.eval_env.get_edge_types(),
                               min_ht=self.config.h_t, max_ht=self.config.h_t, min_ft=self.config.f_t,
                               max_ft=self.config.f_t, hyperparams=self.hyperparams)
                ...
                nodes = batch[1]
                timesteps_o = batch[2]
                traj_pred = self.model.generate(test_batch, node_type, num_points=self.config.f_t, sample=20,bestof=True, sampling=sampling, step=step) # B * 20 * 12 * 2
        ...
        full_path = f'../results/model{self.config.f_t}.txt'
        if not os.path.exists(full_path):
            # 创建空文件
            with open(full_path, 'w') as file:
                file.write("")
        with open(full_path, 'a') as file:
            file.write(f"Epoch {epoch} Best Of 20: ADE: {ade} FDE: {fde}" + '\n')
    ...
    def _build_encoder_config(self):
        ...
        if self.config.eval_mode:
            ...
            # self.checkpoint = torch.load(osp.join(self.model_dir, f"version_2_epoch{epoch}.pt"),
            #                              map_location="cpu")
    ...
    def _build_train_loader(self):
        ...
        self.train_dataset = EnvironmentDataset(train_env,
                                           ...
                                           min_future_timesteps=self.config.f_t,
