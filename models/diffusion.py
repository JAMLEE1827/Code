...
#             nn.ReLU(),
#             nn.AdaptiveAvgPool2d((12, 512))
#         )
#
#     def forward(self, x, map_image):
#         map_features = self.conv_layers(map_image)
#         map_features_pooled = map_features.mean(dim=1)
#         map_features_repeated = map_features_pooled.repeat(x.size(0), 1, 1)
#
#         return map_features_repeated
...
class TransformerConcatLinear(Module):
    ...
    def __init__(self, point_dim, context_dim, tf_layer, residual):
        super().__init__()
        self.residual = residual
        self.pos_emb = PositionalEncoding(d_model=2*context_dim, dropout=0.1, max_len=48)
    ...
    def forward(self, x, beta, context):
        batch_size = x.size(0)      # (B, 12, 2)
        beta = beta.view(batch_size, 1, 1)          # (B, 1, 1)
        context = context.view(batch_size, 1, -1)   # (B, 1, F)

        time_emb = torch.cat([beta, torch.sin(beta), torch.cos(beta)], dim=-1)  # (B, 1, 3)
        ctx_emb = torch.cat([time_emb, context], dim=-1)    # (B, 1, F+3)
        x = self.concat1(ctx_emb,x)     # (B, 12, 2*F)

        # ######
        # map = self.map_feature_extractor(x, image_tensor)
        # x = self.cross_attention(x, map)
        # ######
