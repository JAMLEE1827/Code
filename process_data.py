...
# Process Vessel
for desired_source in ['vessel']:
    for data_class in ['train', 'val', 'test']:
        env = Environment(node_type_list=['PEDESTRIAN'], standardization=standardization)
        attention_radius = dict()
        attention_radius[(env.NodeType.PEDESTRIAN, env.NodeType.PEDESTRIAN)] = 0.5
            ...
            for file in files:
                if file.endswith('.txt'):
                    ...
                    if data_class == 'train':
                        ...
                        # for angle in angles:
                        scene.augmented.append(augment_scene(scene, 0))
        ...
        if len(scenes) > 0:
            with open(os.path.join(data_folder_name, '_'.join(['vessel', data_class]) + '.pkl'), 'wb') as f:
